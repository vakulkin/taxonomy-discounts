...
class TaxonomyDiscounts {
	...
	public function get_applicable_discounts( $product ) {
		...
		foreach ( $discounts as $discount ) {
			...
			if ( $brands && $categories ) {
				...
				$product_categories = $this->get_product_terms( $product->get_id(), $this->product_taxonomy );
				$product_cat_set = array_flip( $product_categories );
				$has_category = false;
				foreach ( $categories as $cat ) {
					if ( isset( $product_cat_set[ $cat ] ) ) {
						$has_category = true;
						break;
					}
				}
				...
				if ( $has_category ) {
					...
					$product_brands = $this->get_product_terms( $product->get_id(), $this->product_brand );
					$product_brand_set = array_flip( $product_brands );
					$has_brand = false;
					foreach ( $brands as $brand ) {
						if ( isset( $product_brand_set[ $brand ] ) ) {
							$has_brand = true;
							break;
						}
					}
	...
	public function apply_taxonomy_discounts( $price, $product ) {
		...
		if ( ! empty( $applicable_discounts ) ) {
			...
			if ( $regular_price > 0 ) {
				$discounted_price = $regular_price * ( 1 - $discount_percent * 0.01 );
